...
> 💡 **ひとことで言うと**
> 大きなAI投資の前に、会社の準備度を25問の「はい/いいえ」で測る健康診断のページである。検査の数値が悪いまま激しい運動を始めると体を壊すように、態勢が整わないまま大型投資をすると失敗しやすい。診断の点数で「投資してよいか、まだ早いか」を判定する。

## AI-Readyはツールの導入度ではなく組織の受け入れ態勢で測る

用語集「AI-Ready」(AIをすぐ成果につなげられる受け入れ態勢)かどうかは、「何を導入したか」では測れない。モデルとツールはすぐ古くなる(陳腐化する)。複利のように積み上がるのは、構造化されたデータ・分解された業務・検証できる人・統制の仕組み・乗り換えられる契約という、組織側の資産である。本ページではこれを次の5資産として定義し、各5問・計25問のYes/Noで自己診断する。

1. **データ・ナレッジ(社内の知識)の構造化** — AIが読める形で社内知が存在するか
2. **業務フローの文書化・分解可能性** — 業務をAIに渡せる単位に切れるか
3. **人材の検証・委任能力** — AIの出力を評価し、任せる範囲を判断できるか
4. **ガバナンス(統制の仕組み)** — リスクを把握し、止められる仕組みがあるか
5. **調達・契約の可換性(いつでも乗り換えられること。用語集「可換性」)** — 特定ベンダー(製品を売る事業者)・特定モデルに縛られず乗り換えられるか

なお、この5資産と25問の設計、後述の実施要領・役員用の5問・判定基準は、複数の成熟度フレームワーク(評価の枠組み)を参考に編集部が独自に組み立てたものである。特定の調査機関の公式診断ではない。
...
- **①データ・ナレッジの構造化**: AIの精度を決めるのはモデルではなく食わせるデータである。AI-readyなデータ基盤を欠くプロジェクトの大量放棄を予測する見解もある(独立した調査ではなく、ITベンダー関係者がIT誌へ寄稿した記事経由の予測である点に留意)[src-2026-011]
- **②業務フローの文書化・分解可能性**: 日本は個人利用は高い一方、部署の業務プロセスへの組み込みが弱い [src-2026-046]。組み込みの前提は、業務が文書化され「AIに渡せる単位」に分解できることである
- **③人材の検証・委任能力**: 国内調査ではスキル・リテラシー(基礎知識)の不足が活用課題の最多であり [src-2026-051]、AIの出力を検証できる人(用語集「検証者」)がいなければ導入は事故の入口になる
...
5資産という切り方の背景には、3つの調査がある。MIT CISR(米マサチューセッツ工科大学の研究センター)の4段階エンタープライズAI成熟度モデル(企業全体のAI成熟度を4段階で測る)[src-2026-001]・BCG(米コンサルティング会社)の成熟度4分類 [src-2026-002]・Oxford EconomicsとServiceNow(ITベンダー=IT製品を売る企業)の共同調査による5次元のAI成熟度指数 [src-2026-006] である。いずれも技術導入ではなく組織能力で成熟度を測る点で共通する。なお同指数では、AI投資の拡大と並行して平均成熟度スコアが低下する局面が観測されている [src-2026-006]。技術の進化で要求水準自体が上がるためである。本診断も、定点ではなく半年ごとの再測定を推奨する。

## 25問チェックリスト

各問にYes/Noで答える。「迷ったらNo」が原則である。

この表の見方(以下5資産の表に共通): 左から問いの番号・問い・Yesと言える状態の例・Noだったときの最初の一手が並ぶ。まず「問い」に答え、Noなら右端の列を次の行動にする。
...
| 18 | AIが関与した成果物で問題が起きた際の報告・対応ルートがあるか | インシデント(事故)の報告先と手順が定義済み | 既存の情報セキュリティ報告ルートにAIを追加 |
...
| # | 問い | Yesの状態の例 | Noなら最初にやること |
|---|------|--------------|---------------------|
| 21 | AI関連の外部委託で、データ・プロンプト(AIへの指示文)・業務知識が自社に残る契約になっているか | 成果物・中間生成物の権利帰属が明記 | 既存契約の権利帰属条項を棚卸しする |
...
Yesの数を数える。以下の3段階の切り方は、各種成熟度モデルの段階構造 [src-2026-001] [src-2026-002] を参考に編集部が置いた便宜的な区分であり、統計的に検証された閾値(判定の境目の数値)ではない。判定は「基盤未整備」「部分的Ready」「スケール準備完了(全社展開に進める状態)」の3段階である。

この表の見方: 自分のYes数が当てはまる行を見る。右の2列が、いまの状態と次の打ち手である。
...
重要なのは合計点よりも**5資産ごとのYes/No分布**である。1つの資産が0〜1点なら、他が満点でもそこがボトルネック(全体の足を引っ張る詰まり)になる。特に資産①が低い場合は、他資産への投資効果が出にくい。AI-readyなデータを欠くプロジェクトの6割は放棄されるという予測(「なぜこの5資産か」の節で触れた、寄稿記事経由の予測の具体値)[src-2026-011] とも整合する。

### 採点結果を投資判断につなぐ

診断結果は、投資判断の5択(本格投資/使い捨て構築/実験/待つ/やらない。詳細はロードマップ編「投資判断の5択」)の受付ゲート(稟議を受け付けるかどうかの判定基準)として使える。「使い捨て構築」とは、捨てる前提で安く小さく作る選択肢のことである(用語の整理は用語集「使い捨て構築」を参照)。以下の接続と金額の目安に出典はなく、実務上の経験則である。自社の決裁規程に合わせて読み替えてほしい。
...
診断結果は次の1枚に集約して経営会議に出す。様式・記載例とも編集部が作成した架空の例である。レーダーチャート(5項目を五角形で示すグラフ)は、5資産×各5点満点で描く。AI施策が複数走る段階では、ポートフォリオ(施策全体の一覧)の分布の行を加える運用も検討する。分布とは成功・継続・中止・学習回収の内訳である(この運用の詳細はロードマップ編「AIDXのリターン構造」で説明している)。
...
この経営報告1枚の様式(記入欄付きMarkdown)は、本サイトの「テンプレート」ページからダウンロードできる。

## 日本企業の現在地 — 自社の相対位置の参考に

構造として読み取れるのは三点である。第一に、日本の大企業では導入そのものは飽和に近づいたが、全社員への利用浸透には大きな段差がある。第二に、利用しても成果創出で米国に後れを取り、格差は拡大傾向が報告されている。第三に、その背景には前節「なぜこの5資産か」で挙げた構造が重なる。すなわち、個人利用止まりでプロセスに組み込まれない・検証できる人材が足りない・外部委託への依存で内製の余地が乏しい、の3つである。具体的な数値は下のデータボックスを参照されたい。
...
本診断の段階を直接測った調査はない。それでも上記のデータからは、日本の大企業の多くは「部分的Ready」に相当すると読める。導入率(本診断の25問では問うていない指標)は高くても、資産②(プロセス組み込み)・資産③(検証能力)・資産⑤(内製・可換性)のNoが多い。この偏りが上記データと整合的である。
...
- **今すぐ**【経営】: 判定結果と5資産の分布を経営会議で確認し、推進オーナー(問19)を任命する。使う道具: 前掲の経営報告1枚様式。完了条件: オーナーの氏名が経営会議の議事録に残る。社長直轄体制の企業で期待以上の効果の比率が高いという結果(前掲61%)[src-2026-048] には注意がいる。これは実施企業側の比率だけで、実施していない企業との比較値は報道になく、要因と断定する根拠にはならない。推進力のある企業ほど社長直轄を選ぶ、という逆向きの説明(選択効果・逆因果)も排除できない。それでも、オーナー不在のまま各論に入るより任命を先にすることを推奨する。
...
- **能力向上を待て**【CFO(財務担当役員)】【経営】: 全社一斉のプロセス再設計や大型のAI基盤投資は、資産①と④が「部分的Ready」水準に達するまで待つ。使う道具: 前掲の投資ゲート。完了条件: ゲート通過を「本格投資」稟議の前提条件として決裁規程・稟議様式に明文化する。基盤未整備のまま進めた投資は放棄リスクが高いと指摘されている [src-2026-011]。
...
2026-06-12 平易化改修済み
2026-06-12 検品反映: 「6割」の参照を前掲扱いから「節名+寄稿経由の具体値」に修正(数字の初出位置の誤解を解消)/「劣後し」削除/「問の外側」を平文化/「ベンダー寄稿経由」を読者向けに展開/CFO初出言い換え。事実・数値・出典タグは不変。
-->
