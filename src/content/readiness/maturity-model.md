...
> 💡 **ひとことで言うと**
> AIのお試し導入をいくつ並べても、会社の数字は良くならない。数字が動くのは、お試しを卒業して、ふだんの仕事の手順そのものにAIを組み込んだときである。ダイエット器具を買い揃えることと、毎日の習慣を変えることの違いに近い。

## 成熟度は導入数ではなく段階で測る

AIの成熟度は、導入したツールの数でもパイロット(本格導入前の試験運用)の本数でも測れない。測るべきは「変革がどの段階まで進んだか」である。分水嶺(結果を分ける境目)は、パイロットを増やす段階から、AIが標準業務フローに組み込まれた「AIの働き方」へ移行できたかどうかにある。MIT CISR(米マサチューセッツ工科大学の研究センター)の追跡調査は、最大の財務インパクトがパイロット段階(Stage 2)から「AIの働き方」段階(Stage 3)への移行で得られると結論している [src-2026-001]。財務インパクト(会社の数字への効果)を生むのは、ツールを増やすことではなく段階の移行である。
...
以下の4段階は、次の3つを編集部が統合した独自の区分であり、特定調査の公式区分ではない。MIT CISRの4段階モデル [src-2026-001]・BCG(米コンサルティング会社)の成熟度4類型 [src-2026-002]・Oxford EconomicsとServiceNow(ITベンダー=IT製品を売る企業)の共同調査が用いる5次元の成熟度測定 [src-2026-006] である。いずれの調査も、技術導入度ではなく組織の働き方で成熟度を測る点で共通する。

この表の見方: 縦に4つの段階、横にその段階の特徴・はまりやすい罠・抜け出す条件が並ぶ。自社が当てはまる行を探し、右端の「次へ進む条件」を読む。
...
MIT CISRの調査では、Stage 3〜4の企業は業界平均を上回る財務パフォーマンスを示し、Stage 1〜2の企業は下回る [src-2026-001]。境界線はちょうど「パイロット止まり」と「業務に組み込まれたAIの働き方」の間に引かれている。ただしこれは横断面調査(ある一時点での比較)で見えた相関にすぎない。移行したから儲かったのか、余力のある企業ほど移行できたのか(選択効果・逆因果)は区別できない。因果と断定はできない。それでも「パイロットを何本足しても、業務本体が変わらなければ財務数値は動かない」という仕組みの説明としては筋が通る。
...
成熟度を定点スコアで測ると、努力しているのに点が下がる現象が起きる。前掲の共同調査では、AI投資が増えているにもかかわらず平均成熟度スコアが前年から低下した。背景として挙げられているのは、スキルギャップ(必要な技能と現状の差)の拡大やデータプライバシー(個人データ保護)への懸念である。加えて、エージェント型AI(目標を与えると自分で段取りを進めるAI。用語集「AIエージェント」)など新技術の登場で、要求水準そのものが上がった [src-2026-006]。
...
構造として読めるのは、導入は進んだが段階移行が止まっている、という形である。ツールのアカウントは配られても、業務の手順書が導入前と変わっていないなら、それは段階移行が止まっている状態である。大企業に限れば、導入はほぼ飽和した。一方で全社員への利用浸透は一部にとどまり、「部署の業務プロセスに組み込まれている」とする回答は国際比較で低い。段階の分布を直接測った日本の調査はない。それでもこれらの構造から推すと、日本の大企業の多くは**パイロット期**に滞留していると読める。先頭集団がようやく、配置転換を伴う組み込み期の入口に立った段階である。世界の分布が組み込み期側へ移行し続けるのと対照的に、差は導入率ではなく段階移行の速度で開いている。
...
移行の最初の90日で何をやるか。体制・予算・決裁レンジ(決裁できる金額の幅)の目安に出典はなく、実務上の経験則である。中堅(数百名規模)と大企業(数千〜数万名規模)の2レンジで示す。自社の決裁規程に合わせて読み替えること。初動の順番(各ページの「今すぐ」アクションをどの順で始めるか)は、ロードマップ編「最初の90日」を参照。90日単位の手を1年の四半期計画につなぐ展開は、ロードマップ編「1年プランと3年シナリオ」を参照。本表が扱うのは、段階を1つ進めるための90日である。

この表の見方: 左端で自社の現在地に当てはまる行を選び、横にやることと、企業規模ごとの体制・費用の目安を読む。
...
- **1年以内**【CFO(財務担当役員)】AI支出を「パイロット予算」と「組み込み予算(フロー書き換え・基盤・教育)」に区分して把握する。道具: 費目別のAI支出集計。完了条件: 区分比率が算出され、パイロット偏重なら是正方針が経営会議に上がる
